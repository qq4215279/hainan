	...
	IndexPage.openDetail = function(id) {
		var index = layer.open({
			...
			fix : false, //不固定
			maxmin : true,
			content : App.ctxPath + 'membercard/apply/detail/' + id
		});
	...
	IndexPage.openLog = function(id) {
		var index = layer.open({
			...
			fix : false, //不固定
			maxmin : true,
			content : App.ctxPath + 'membercard/apply/log/' + id
		});
	...
	$(function() {
		...
		var table = new BSTable(IndexPage.id, "membercard/apply/list",defaultColunms);
