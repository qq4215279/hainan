	...
	IndexPage.openDetail = function(id) {
		var index = layer.open({
			...
			fix : false, //不固定
			maxmin : true,
			content : App.ctxPath + 'admission/apply/detail/' + id
		});
	...
	IndexPage.openLog = function(id) {
		var index = layer.open({
			...
			fix : false, //不固定
			maxmin : true,
			content : App.ctxPath + 'admission/apply/log/' + id
		});
	...
	$(function() {
		...
		var table = new BSTable(IndexPage.id, "admission/apply/list",defaultColunms);
