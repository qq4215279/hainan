...
Menu.openAddMenu = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: App.ctxPath + 'menu/menu_add'
    });
...
Menu.openChangeMenu = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: App.ctxPath + 'menu/menu_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Menu.delMenu = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(App.ctxPath + "menu/remove", function (data) {
                App.success("删除成功!");
                Menu.search();
            }, function (data) {
