...
Role.initColumn = function () {
    var columns = [
        ...
        {title: '上级角色', field: 'pName', align: 'center', valign: 'middle', sortable: false},
        {title: '别名', field: 'tips', align: 'center', valign: 'middle', sortable: false}]
    return columns;
};
...
Role.openAddRole = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: App.ctxPath + 'role/role_add'
    });
...
Role.openChangeRole = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: App.ctxPath + 'role/role_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.delRole = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(App.ctxPath + "role/remove", function () {
                App.success("删除成功!");
                Role.table.refresh();
            }, function (data) {
        ...
        App.confirm("是否删除角色 " + Role.seItem.name + "?",operation);
    }
};
...
Role.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: App.ctxPath + 'role/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
