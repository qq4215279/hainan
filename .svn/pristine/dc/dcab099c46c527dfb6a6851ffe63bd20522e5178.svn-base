...
public class KaptchaController extends BaseController {
     ...
     */
    @RequestMapping("/" + UploadConstants.AVATAR + "{pictureId}")
    public void renderPicture(@PathVariable("pictureId") String pictureId, HttpServletRequest request, HttpServletResponse response) {
        String path = manageProperties.getFileUploadPath()+ UploadConstants.AVATAR + pictureId + ".jpg";
        try {
            ...
            //如果找不到图片就返回一个默认图片
            try {
                response.sendRedirect(request.getContextPath() + "/static/img/gonhui-64.jpg");
