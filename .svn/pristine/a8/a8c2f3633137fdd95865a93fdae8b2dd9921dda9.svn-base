...
//	}
    ...
    /* 拆分标准: 小集合长度 */
    private final static double groupLength = 1000.0;

    /**
     * 大集合拆分
     * groupLength: 小集合的长度(当 list的长度不是groupLength的整数倍时，最后一个的长度为它俩的余数)
     * groupNum: 拆分后的小集合组的长度
     * <p>
     * 按照固定长度拆分大集合
     * 如: list 长度为120，groupLength 为10，则 groupNum(每个小集合的长度) 为12，groups 为长度是 12 的集合组
     * list 长度为126，groupLength 为10，则 groupNum(每个小集合的长度) 为13--最后一个小集合长度为 126 % 10 = 6，groups 为长度是 13 的集合组
     *
     * @param list ：带拆分的集合
     * @return groups：拆分的结果
     */
    private static List<List<?>> spiltList(List<?> list) {
        int groupNum = (int) Math.ceil(list.size() / groupLength);

        List<List<?>> groups = new LinkedList<>();
        for (int i = 0; i < groupNum; i++) {
            int fromIndex = i * (int) groupLength;
            int toIndex = (i + 1) * (int) groupLength;
            if (list.size() % groupLength > 0 && i == groupNum - 1) {
                toIndex = fromIndex + list.size() % (int) groupLength;
            }
            List<?> group = list.subList(fromIndex, toIndex);
