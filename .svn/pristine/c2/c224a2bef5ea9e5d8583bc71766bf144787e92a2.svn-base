...
OptLog.initColumn = function () {
    return [
        ...
        {title: '时间', field: 'createtime', align: 'center', valign: 'middle', sortable: true,
            ...
            var a = dateDisplay(row.createtime);
            return a;
        	}
...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            Feng.infoDetail("日志详情", data.regularMessage,1250);
        }, function (data) {
            Feng.error("获取详情失败!");
        });
        ajax.start();
...
OptLog.formParams = function() {
    ...
    queryData['beginTime'] = frDate2time($("#beginTime").val());
    queryData['endTime'] = toDate2time($("#endTime").val());
